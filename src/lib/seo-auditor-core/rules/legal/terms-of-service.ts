...
import { defineRule, pass, warn, fail } from '../define-rule';
...
export const termsOfServiceRule = defineRule({
  ...
  run(context: AuditContext): RuleResult {
    ...
    if (foundLinks.length > 0) {
      ...
      return pass('legal-terms-of-service', `Terms of service link found${inFooter ? ' in footer' : ''} (${foundLinks.length} link${foundLinks.length > 1 ? 's' : ''})`, {
    ...
    if (likelyNeedsToS) {
      return warn('legal-terms-of-service', 'No terms of service link found - recommended for this site type', {
    ...
    return pass('legal-terms-of-service', 'No terms of service link found (may not be required for this site type)', {
      hasTermsOfService: false,
    });
  },
});
