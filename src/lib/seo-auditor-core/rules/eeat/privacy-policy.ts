...
export const privacyPolicyRule = defineRule({
  ...
  run(context: AuditContext): RuleResult {
    ...
    if (foundLinks.length > 0) {
      ...
      return pass('eeat-privacy-policy', `Privacy policy link found${inFooter ? ' in footer' : ''} (${foundLinks.length} link${foundLinks.length > 1 ? 's' : ''})`, {
    ...
    return warn('eeat-privacy-policy', 'No privacy policy link found - important for trust and legal compliance', {
